  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(async (params) => {
      ...
      if(this.table == 'admin-attestation'){
        ...
        // osid_data.forEach(element => {
        //   this.generalService.postData("PrerakV2/search", {"filters":{"osid":{"contains":element?.osid}}}).subscribe(
        //     (res) => {
  ...
  downloadPrerakCSVFile() {
   ...
   downloadAGCSVFile() {
    ...
    this.model.forEach(async (element) => {
      arr = [];
      let obj = [];
      let prerak_obj = osid_data.find(o => o.osOwner == element['osOwner'][0]);
